...
import static com.jgcomptech.adoptopenjdk.APISettings.*;
    ...
    public static void processRelease(final JavaRelease baseRelease,
        ...
        for (final SimpleAsset binaryAsset : release.getBinaryAssets()) {
            ...
            if(currentAssets.contains(binaryAsset.getAssetName())) {
                //System.out.println("--> Skipping " + releaseName + " Asset: " + binaryAsset.getFilename() + " - Already Exists!");
                continue;
            }

            //Check if file matches java version
            if(!binaryAsset.isMajorVersion(baseRelease.getMajorBuild())) {
                //System.out.println("--> Skipping " + releaseName + " Asset: " + binaryAsset.getFilename() + " - Not Java " + baseRelease.getMajorBuild() + '!');
                continue;
            }

            //Check if file matches jvm type, hotspot or openj9
            if(!binaryAsset.isAssetJVMType(jvmType)) {
                //System.out.println("--> Skipping " + releaseName + " Asset: " + binaryAsset.getFilename() + " - Not Java " + jvmType + '!');
                continue;
            }

            //Check if the file matches release type, JDK or JRE
            if(binaryAsset.getReleaseType() != type) {
                //System.out.println("--> Skipping " + releaseName + " Asset: " + binaryAsset.getFilename() + " - Not Java " + type + '!');
                continue;
            }
